...
const MobileBottomNav = () => {
    ...
    return (
        <>
            ...
            <span onClick={() => window.scrollTo({ left: 0, behavior: 'smooth', top: 0 })} ref={navRef} className={`fixed duration-500 hidden py-[10px] xs:flex items-center justify-center text-3xl font-bold hover:text-[#08c] cursor-pointer text-white w-[50px] bg-[#404040bf] h-[45px] right-[10px] ${isFixed ? 'bottom-0 opacity-100' : '-bottom-[70px] opacity-0'}`}><FaAngleUp /></span>
        </>
    );
};
